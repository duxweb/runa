...
func (provider *provider) Register(ctx runaprovider.Context) error {
	...
	registry.RegisterDriver(opts.driverName, newStore(client, opts, ownsClient))
	return nil
}
...
func defaultOptions() options {
	return options{driverName: defaultDriverName, useName: defaultSharedName, addr: defaultRedisAddr, prefix: lock.DefaultRedisPrefix}
}
...
func newClient(opts options) *goredis.Client {
	return goredis.NewClient(&goredis.Options{Addr: opts.addr, Username: opts.username, Password: opts.password, DB: opts.db, DialTimeout: opts.dialTimeout, ReadTimeout: opts.readTimeout, WriteTimeout: opts.writeTimeout, PoolSize: opts.poolSize, MinIdleConns: opts.minIdle})
}
