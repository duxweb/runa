...
Redis 驱动可以用自己的 Provider 注册。`cache/redis` 获取 client 的顺序是：`Client(client)` 注入、`Addr(...)` 等显式选项、`[cache.redis]` 功能配置、共享 `[redis]` / `[redis.<name>]` 配置，最后回退到 `127.0.0.1:6379`。
...
仍然可以用 `cacheredis.Driver(client, cache.Prefix(...))` 手工传入 client，或用 `cacheredis.Provider(cacheredis.Client(client))` 注入。注入的 client 不由 Runa 关闭；Provider 自建的 client 会随应用生命周期关闭。
