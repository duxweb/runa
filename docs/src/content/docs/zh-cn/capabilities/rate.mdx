...
内置 `memory` 驱动适合单进程限流。多实例部署时，安装 `rate/redis`，再用 `rate.Use("redis")` 让指定 limiter 使用它。
...
`rate/redis` 也支持 `Client(client)` 注入共享 Redis client。注入的 client 不由 Runa 关闭；Provider 自建的 client 会随应用生命周期关闭。
