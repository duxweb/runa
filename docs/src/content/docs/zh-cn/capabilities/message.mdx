...
Redis、NATS、AMQP、MQTT 驱动包都提供 `Provider(...)` 做配置化接入，同时保留 `Driver(conn/client, ...)` 作为手工接线逃生口。注入的 client/conn 不由 Runa 关闭；Provider 自建的连接会随应用生命周期关闭。驱动配置会从 `[message.<driver>]` 回退到共享 `[redis]`、`[nats]`、`[amqp]` 或 `[mqtt]`。
