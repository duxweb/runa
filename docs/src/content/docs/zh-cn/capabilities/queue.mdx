...
`retention` 会启用失败任务自动清理。内置 `memory` 驱动和 `queue/redis` 支持 `Purge`，会按保留期删除 failed 任务；默认保留 `7d`，设置 `retention = "0"` 或 `queue.Retention(0)` 表示不清理。AMQP 驱动不提供 failed 列表和清理能力，所以不支持 retention 清理。
...
生产环境应使用队列专用 Redis，或至少确保 Redis 配置为 `maxmemory-policy noeviction`。不要把队列数据放进配置了 `allkeys-lru` 等淘汰策略的缓存 Redis；内存压力下 job body 被 Redis 静默驱逐就等于丢任务。payload 也应保持小体积：大文件、大文档放外部存储，队列里只放引用，建议单个 job body 控制在 `64KB` 以下。
...
- **默认 `Concurrency = 10`**：开箱每个 worker 最多同时跑 10 个任务。可用 `queue.Concurrency(N)` 调整；IO 密集型任务可以设置到几十或上百，但要看下游服务承压能力。
...
- **失败任务默认保留 7 天**：支持 `Purge` 的驱动会自动清理旧 failed 任务。设置 `queue.Retention(0)` 或非正配置值可关闭清理、永久保留 failed 任务。
