...
`route.Service[T](ctx)` is a lower-level escape hatch for services injected into the route context. Most application code can start with capability `Default()`.
