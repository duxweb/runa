...
// Provider registers a Redis cache driver. It can use an injected client,
// explicit connection options, cache.redis config, redis shared config, or defaults.
func Provider(items ...Option) runaprovider.Provider {
	return &provider{items: append([]Option(nil), items...)}
}

func (provider *provider) Name() string { return "cache.redis" }

func (provider *provider) Priority() int { return 10 }

func (provider *provider) Register(ctx runaprovider.Context) error {
	registry, err := runaprovider.Invoke[*cache.Registry](ctx)
	if err != nil {
		return err
	}
...
func defaultOptions() options {
	return options{driverName: defaultDriverName, useName: defaultSharedName, addr: defaultRedisAddr, prefix: cache.DefaultRedisPref, ttl: cache.DefaultTTL}
}
